export const PROJECTS = [
  {
    ...
    problemStatement: 'During the Last Glacial Maximum (LGM), approximately 20,000 years ago, Earth\'s atmosphere held 80 to 100 ppmv less CO₂ than during warm interglacial periods. Ice cores make this deficit clear. What they cannot tell you is where the carbon went. The terrestrial biosphere was smaller during the ice age, which rules out land as the sink. Only one reservoir on Earth was large enough: the deep ocean. The scientific mechanism explaining how the ocean stored that carbon was contested and, more importantly, locked inside academic literature that no general audience would ever read. That was the design problem.',
    problemMetrics: [
      { value: '80 to 100 ppm', label: 'atmospheric CO₂ deficit during the Last Glacial Maximum, confirmed by Antarctic ice core records' },
    ...
    process: [
      ...
      },
      {
        ...
        body: 'The original thesis figures were generated in Python for scientific publication: functional, accurate, and unreadable to a non-specialist. Each was rebuilt from scratch as a public-facing visual. Deep ocean blues and neutral overlays anchor the spatial context. High-contrast paired comparison maps show Pre-Industrial versus LGM states side by side. A consistent visual grammar for radiocarbon age, transport strength, and sea ice extent carries across all 13 slides without requiring a legend re-read on each.',
    ...
    solutionMetrics: [
      ...
      { value: '4 runs', label: 'MITgcm experiment configurations allowing mechanism separation: PI, LGM with sea ice, LGM without sea ice, combined LGM' },
    ],
...

  {
    ...
    problemMetrics: [
      ...
      { value: '18 to 24 months', label: 'grid reinforcement lead time at constrained motorway service areas, independent of planning decisions' },
    ],
    ...
    researchMethods: [
      ...
      'High-power charge point audit: Zap-Map API and NESO public data, 150 kW and above units mapped against motorway service area locations per corridor',
    ...
    process: [
      {
        ...
        body: 'Identified the eight corridors carrying approximately 70% of UK HGV freight: M1, M6, M62, M4, A14, M5, A1(M), and M8, using DfT road traffic count data. Mapped every high-power charge point (150 kW and above) against each corridor and calculated viable refuelling intervals for 19-tonne and 44-tonne zero emission configurations. The audit found that charge point density varies by a factor of more than four between the best and worst-served corridors.',
        img: 'Corridor Map  ·  Charge Point Audit',
      },
      {
        step: '02', label: 'Gap Modelling', slideIdx: 7,
        body: 'Built a grid capacity constraint layer over the corridor map using NESO distribution network data. The most significant finding was that several high-dwell motorway service areas sit in areas where the local distribution network cannot support the required additional load without reinforcement. That process takes 18 to 24 months from application to energisation, independent of planning or funding decisions. On those locations, the gap is not a deployment delay. It is a structural constraint with a fixed lead time.',
        img: 'Gap Model  ·  Grid Constraint Layer',
      },
      {
        step: '03', label: 'Viability Scoring', slideIdx: 9,
        body: 'Developed a viability index for each corridor across three dimensions: charge availability ratio (viable chargers per freight movement), maximum gap distance between viable charge points, and grid constraint risk score (proportion of service areas in constrained zones). Corridors scoring below the viability threshold represent structural barriers, not slow deployment that will self-correct as investment arrives. They are locations where infrastructure readiness requires active intervention at the grid level first.',
        img: 'Viability Index  ·  Corridor Scoring Matrix',
      },
      {
        step: '04', label: 'Decision Framework', slideIdx: 11,
        body: 'Translated the corridor scores into a prioritisation framework for two distinct audiences who are often planning independently. Fleet operators need to know which routes can support ZE fleet operation now and which require phased introduction. Infrastructure planners and DSOs need to know where grid reinforcement should be sequenced before charger deployment begins. A single dataset, structured for two different decision contexts. That dual-audience design was the most operationally useful output of the analysis.',
    ...
    solutionBody: 'Charge Corridor identifies where ZE HGV viability breaks down on the eight UK freight corridors carrying 70% of road goods. Each corridor is scored against charge availability, grid capacity at dwell locations, and the maximum operational gap a 44-tonne ZE configuration can tolerate without a range failure. The grid constraint analysis adds the dimension most gap analyses miss: where the constraint is structural and has a fixed lead time, not simply an investment shortfall. The framework produces two prioritised views, one for fleet operators deciding route sequencing, and one for infrastructure planners and DNOs deciding where grid reinforcement should precede charger deployment.',
    solutionMetrics: [
      ...
      { value: '2 audiences', label: 'fleet operators and infrastructure planners, one dataset structured into two decision-ready views' },
    ],

    reflectionQuote: 'Zero emission freight is an infrastructure problem before it is a vehicle problem. The vehicles exist. The grid constraints are real and have a timeline that no planning decision can compress.',
...

  {
    ...
    solutionBody: 'Shore Power produces a four-dimension readiness score, a technology pathway recommendation matched to the vessel fleet profile at each port, and a prioritised infrastructure investment brief. Applied across six major UK port estates (Felixstowe, Southampton, Dover, Immingham, Liverpool, Tilbury), the framework shows that port decarbonisation constraints are not uniform. Some ports are grid-constrained; some are land-constrained; some operate vessel fleets with no commercially available zero emission alternative yet. The main finding is that short-sea and ferry routes have no technical barrier to shore power deployment now, while deep-sea container routes require a different investment sequence aligned to hydrogen and methanol infrastructure timelines.',
    solutionMetrics: [
      ...
      { value: '2 to 3 yr vs 5 to 7 yr', label: 'shore power versus hydrogen bunkering lead times, the infrastructure sequencing gap that drives investment prioritisation' },
    ],
...

  {
    ...
    problemMetrics: [
      ...
      { value: '4 to 8 years', label: 'whole-life break-even range versus diesel equivalent, depending on energy tariff and duty cycle intensity' },
      { value: '18 to 36 months', label: 'typical lead time from fleet commitment to first ZE vehicle delivered and operational' },
    ],
    ...
    process: [
      ...
      },
      {
        ...
        body: 'Built a whole-life cost comparison model across diesel, battery electric, and fuel cell configurations. The break-even analysis spans 4 to 8 years depending on energy tariff assumptions and duty cycle intensity. The most sensitive variable is not vehicle purchase price. It is the energy tariff. Operators on flexible industrial tariffs in the 12 to 15 pence per kWh range reach break-even in 4 to 5 years. Operators on standard grid rates above 24 pence per kWh are looking at 7 to 8 years or longer, which changes the board-level investment case entirely.',
    ...
    solutionMetrics: [
      ...
      { value: '4 to 8 yr', label: 'whole-life break-even range: energy tariff is the decisive variable, not vehicle purchase price' },
      { value: '4 phases', label: 'transition roadmap with decision gates — route and depot validation always precede vehicle procurement' },
    ],
    ...
    reflectionBody: 'The most consistently useful output was the whole-life cost model with energy tariff sensitivity. Most internal business cases operators were presenting to boards treated energy cost as a single number, typically the current retail rate. Showing the break-even range across realistic tariff scenarios, from flexible industrial rates to standard grid prices, changed the conversation from whether ZE is viable to what energy contract is needed to make ZE financially defensible. That reframing from technology question to commercial question was what most operators needed to move from analysis to commitment.',
  },

  {
    ...
    problemStatement: 'Accessible transport is a legal requirement across UK rail, bus, and active travel infrastructure. The reality is uneven and the unevenness is not random. Step-free access, real-time audio information, tactile wayfinding, and accessible waiting environments vary dramatically between operators, modes, and regions. The central problem is that existing accessibility assessments are single-mode by design: Office of Rail and Road audits rail, bus stop accessibility is assessed separately, and active travel infrastructure has its own rating system. No framework combines these to identify where multiple accessibility failures compound for the same population, which is where investment decisions would create the largest per-person improvement in access.',
    problemMetrics: [
      { value: '13.9M', label: 'people in the UK living with a disability (Family Resources Survey 2022 to 23)' },
...

  {
    ...
    problemMetrics: [
      { value: '6 to 19 Sv', label: 'AMOC transport range across the RAPID array observational record at 26.5°N (2004 to present)' },
    ...
    researchMethods: [
      ...
      'Three-scale temporal analysis: short-term variability (annual, 6 to 19 Sv range), medium-term trend (5-year rolling mean, RAPID record), long-term baseline (Caesar proxy reconstruction from mid-20th century)',
    ...
    process: [
      ...
      },
      {
        ...
        body: 'Designed a public-facing AMOC monitoring dashboard concept structured around three temporal views: a 7-day rolling RAPID transport reading in Sv, a 12-month trailing mean trend line with the natural variability envelope shown explicitly, and a long-term status indicator referencing the Caesar proxy baseline. The status indicator uses a three-tier system (stable, declining, structurally weakened) with threshold values drawn from the scientific literature. The dashboard was designed around the principle that the monitoring data should not require a physical oceanography background to interpret, but should not hide the uncertainty that a physical oceanography background would make visible.',
    ...
    solutionBody: 'AMOC Watch is a science communication design framework that takes the RAPID array time series and makes it legible to a climate policy audience without flattening the uncertainty. It separates three temporal scales (annual variability spanning 6 to 19 Sv, 5-year rolling trend, and Caesar proxy long-term baseline) and represents each at the appropriate visual weight. The regional impact mapping translates Sv measurements into UK-relevant climate consequences. The dashboard concept gives climate communicators and policy teams a reference design for working with continuous ocean monitoring data: a 7-day rolling reading, a 12-month trailing mean, and a three-tier status indicator anchored to the scientific literature.',
    solutionMetrics: [
      { value: '3 timescales', label: 'annual variability (6 to 19 Sv), 5-year rolling trend, and Caesar proxy baseline, each visualised separately' },
      { value: '3-tier status', label: 'dashboard indicator: stable, declining, structurally weakened, with thresholds from peer-reviewed literature' },
...

  {
    ...
    problemMetrics: [
      { value: '5.5 to 6.0', label: 'kWh/m²/day average solar irradiance in northern Ghana (Savannah, North East, Upper East regions), among the highest in West Africa' },
    ...
    researchMethods: [
      ...
      'Technology pathway modelling: rooftop solar (under 10 km from grid), mini-grid systems (10 to 50 km from grid), and solar home systems (above 50 km from grid) matched to grid proximity bands',
    ],
    ...
    solutionMetrics: [
      ...
      { value: '3 pathways', label: 'rooftop solar (under 10 km to grid), mini-grid (10 to 50 km), solar home systems (above 50 km), matched to grid proximity' },
      { value: '0.5 to 1.0 kWh', label: 'irradiance advantage of northern Ghana over the south per m²/day, the resource differential investment planning should reflect' },
    ],
